...
A malformed declaration — anything that is not an object of non-negative safe
integers (whole numbers from 0 up to `Number.MAX_SAFE_INTEGER`) — refuses the
stage with `invalid-request` rather than being ignored. An agent that sent one
