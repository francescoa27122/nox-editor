...
4. The first visible line is `view.lineBlockAtHeight(view.scrollDOM.scrollTop - view.documentTop)`; its line number comes from `view.state.doc.lineAt(block.from).number`.
