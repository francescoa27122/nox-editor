...
1. **`CONTRIBUTING.md:23-82`** — the five rules and the feature checklist. Non-negotiable, and this skill does not restate them.
2. **`ARCHITECTURE.md:45-189`** — the layer diagram and where everything lives.
3. **`ARCHITECTURE.md:190-1372`** — the decision log. Before changing anything load-bearing, check whether the current shape is already argued for there. It usually is.
...
The schema closes with `} as const satisfies Record<string, SettingDescriptor>` (`schema.ts:303`) — **both halves are load-bearing**: `as const` keeps literal defaults so `Settings` gets `'eclipse' | 'umbra'` instead of `string`, and `satisfies` validates without widening.
...
`MemoryPlatform` is the fixture, not a mock — the same code path the browser build uses, so a pass means the behaviour works rather than that a stub was configured correctly. Seed with `seedFile`, `seedGitRepo`, `seedGitBase`; simulate outside changes with `externalWrite` / `externalRemove` / `externalRename`.
