...
| Quick-open costs a frame per keystroke on a large project | Measured 2026-08-25 with `npm run bench`: `fuzzyFilter` over a **16,000-path** index is **15.3 ms mean** (12.0 min, 19.8 max) against a 16 ms frame — so on a project that size the filter *is* the frame. The command palette's own ~200 entries cost 0.36 ms and are not affected. Nothing is quadratic; `tests/complexity.test.ts` holds it linear, and linear is the problem: the work is proportional to the index and the index is the whole project. The fixes are all real changes rather than tuning — bound the candidate set before scoring, keep the previous result set and filter within it while the pattern only grows, or move the scan off the main thread. |
