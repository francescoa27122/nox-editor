...
| Plugins get no events | There is no "the buffer changed" or "the selection moved" message, deliberately: a plugin woken per keystroke is the thing being out of process exists to prevent. The cost lands on status items, which can only change when the plugin does something — at startup, on one of its own commands, or on a timer it owns. A readout that tracks the editor live is not buildable on this API, and `examples/plugins/counter/` says so rather than implying otherwise. A push channel at a coarse, debounced granularity ("the active buffer changed", never "a character was typed") is the shape that would keep the gate, and it is unbuilt. |
| Plugins contribute commands and status items, not panels or editor decorations | Sidebar panels and the declarative editor surface are unbuilt. `SidebarView` is a closed union and `Sidebar.svelte`'s `VIEWS` is a hardcoded table, so a plugin panel is a real change to both rather than a registration. Deliberate: the roadmap row asked for four surfaces and this is one, because a plugin API is a permanent compatibility promise and the narrow half is the half worth living with first. |
