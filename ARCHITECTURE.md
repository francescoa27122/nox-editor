...
| Plugin behaviour is verified from disk, never from the screen | The worker transport, custom themes, plugin settings and the config watcher were all driven in the packaged Windows build on 2026-08-29 and all pass — `worker-src 'self' blob:` does permit the blob worker under Tauri's custom-protocol origin, which is what the old "unverified" row here was really asking. But screen-control consent was declined, so every readout was a file: a command's effect on `settings.json`, a plugin's own log, a warning in `diagnostics.log`. Nothing has observed a *pixel* — that a custom theme's accent is drawn, that the Plugins tab renders its rows, that a theme edited while selected repaints. That is the remainder, and it is about appearance rather than behaviour. See `.desktop-pass-report.md`. |
