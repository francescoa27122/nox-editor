...
| Plugins contribute commands, status items and panels, not editor decorations | The declarative editor surface is unbuilt. `SidebarView` is a closed union and `Sidebar.svelte`'s `VIEWS` is a hardcoded table, so a plugin panel is a real change to both rather than a registration. Deliberate: the roadmap row asked for four surfaces and this is one, because a plugin API is a permanent compatibility promise and the narrow half is the half worth living with first. |
