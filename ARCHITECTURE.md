...
| `net.request` is documented as a gate and no command declares it | `permissions.ts:98` justifies `'fs.read': 'allow'` with "it cannot leave the process on its own, and `net.request` is the gate that matters", and `AGENT-PLATFORM.md:218` and `ARCHITECTURE.md` say the same twice more. Enforcement is driven entirely off `command.capabilities`, and **no command in the table declares `net.request`**. The provider that actually sends text off-process reaches `Platform.streamJsonLines` directly rather than through a command, so the guard never runs. Not an exfiltration hole today, and the reason is a different mechanism the docs do not credit: `http.rs`'s `is_loopback` refuses any non-loopback URL and disables proxies. But the argument holding `fs.read` open is currently unenforced, and ROADMAP's "Later: AI" plans remote model support, which is the point at which it stops being theoretical. Found 2026-08-30. |
| `LineStream::partial` is unbounded, and re-scanned per chunk | `agent.rs:277-290` appends 8 KiB at a time into a `String` with no cap and calls `find('\n')` over the whole of it each time, so a child that writes without a newline costs memory without limit and CPU quadratically. It is the reader behind every piped stream Nox supervises, which now includes **tasks**: an approved task whose build script emits a large newline-free stream takes the backend down with every unsaved buffer in it. `MAX_OUTPUT_LINES` cannot help, because the line never arrives. The fix is a cap that emits and resets, plus a tracked search offset. Found 2026-08-30. |
| Four modal overlays do not trap focus, and `CONTRIBUTING.md` says they do | `CONTRIBUTING.md:211` states "Overlays trap focus and are dismissible with Esc". Escape holds everywhere. The trap is implemented in `CommandPalette` and `ContextMenu` only; `ConfirmDialog`, `PromptDialog`, `SettingsPanel` and `KeybindingsPanel` contain no `Tab` handling, and nothing sets `inert` on the shell behind the scrim, so Tab walks out of a modal into the editor underneath. All four assert `aria-modal="true"`, which tells a screen reader the opposite of what is true. One shared action, or `inert` on `.nox-shell` while any overlay is up, fixes it in one place. Found 2026-08-30. |
| A session grant keys on the capability while the prompt names a command | `grantKey` (`permissions.ts:396`) scopes on the resource only for `fs.*` and `buffer.edit`, so two commands declaring the same unscoped capability share one "allow for this session" bucket while the dialog the user answered named just one of them. Live for `buffer.edit`: approving `review.apply` also covers `search.replaceAll`, which is a project-wide replace the dialog never mentioned. `AGENT-PLATFORM.md:271` says grants key on the capability "which is the granularity each is asked at", and they are not asked at that granularity. Found 2026-08-30. |
| Six side-effecting commands declare no capabilities | `plugins.reload` and `lsp.reload` spawn processes, `terminal.toggle` and `terminal.restart` start a shell, `app.checkForUpdates` makes an outbound request, and `notes.new`/`notes.rename` write files. `commands.ts:200` gates on `command.capabilities?.length`, so a command that declares nothing is never guarded for any principal, against a comment two lines up promising "no second path to forget about". `view.reloadWindow` is the sharp one: the permission decision log and every session grant are in memory only, so a plugin can dispatch it and erase the record `AGENT-PLATFORM.md` presents as the audit trail. Found 2026-08-30. |
| Five hardcoded colours in the editor theme | `editor/theme.ts:95,225,230,235,254` write `rgba(...)` literals for selection-match, bracket match, non-matching bracket and the two search-match outlines. Four shadow tokens a custom theme *can* set, so a theme that recolours `accent` gets a bracket whose fill moves and whose outline stays glacial cyan, and `.cm-selectionMatch` has no token at all. DESIGN.md §9 claims "the editor surface and the app chrome physically cannot drift apart". `tests/component-css-tokens.test.ts` scans `src/ui` only. Found 2026-08-30. |
| The blame gutter's column *order* is unasserted | `tests/browser/blame-gutter.test.ts` holds the width three ways and records the mutation that breaks each, but nothing asserts the blame column comes before the line numbers. That ordering is half of what screenshotting found on 2026-08-29, so it is a regression that has already happened once and is still guarded only by a screenshot nobody has to look at. One line against `view.dom.querySelectorAll('.cm-gutter')` in the test that already mounts the real extensions. |
