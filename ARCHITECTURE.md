...
| Agent processes are not sandboxed | A configured agent runs with Nox's own privileges. The permission model governs what it may ask *Nox* to do, not what its own process can reach. A stdio agent is trusted code you chose to run, like a shell plugin. Its lifetime is tied to Nox's only as far as a hook can reach: the renderer's `beforeunload` and the host's `RunEvent::Exit` both kill every agent, terminal and language server, but a host crash runs neither, and a child then outlives Nox. Closing that needs a Windows job object and a Linux death signal, which is FFI this crate does not do yet. |
