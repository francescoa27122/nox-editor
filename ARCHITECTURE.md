...
| `lint` is not a required check on `main` | The ten contexts on the protected branch predate the job (added 2026-08-25). Until `lint` is added to them, a PR that fails it can still be merged — everything else about the job works, and it is green on `main`. |
