...
running anything on a non-user principal's behalf. That one check is sufficient
...
**A command that declares nothing is refused, not waved through.** Until
2026-09-03 the guard was consulted only when `capabilities` was non-empty, so
an absent declaration meant "needs no permission" rather than "does nothing
that needs one". Those are different claims, and nothing checks the second: a
`run` function cannot be asked whether it reaches the OS. A security review on
2026-08-30 found twelve side-effecting commands making it falsely, and the
audit that followed reproduced three escapes through the gap, one of them an
agent reloading the window and erasing the decision log that would have
recorded it. Declaring the twelve fixed the instances and left the class open,
which is what the guard seeing every command closes.
`PermissionService.refuseUndeclared` throws the same `PermissionError` a denial
throws, and records the refusal under its own source, `undeclared`, so an audit
can tell "you did not say what you would do" apart from "policy said no". The
price is that a genuinely capability-free command an agent might legitimately
want, `review.show` or `nav.goToLine`, is refused with them. No shipped example
needs one, and the alternative was an exemption flag that any new command could
set, which is the hole again with a name.
...
- **Large files:** CodeMirror virtualises the viewport and stores the document
  ...
  8 KB and refused with a clear message. Between the two, a buffer over 5 MB
  (`LARGE_FILE_BYTES`, `workspace.ts`) opens in **large-file mode**: it is
  never announced to a language server, gets no gutter diff, and has its
  unsaved-work backup written on a 2 s debounce rather than 400 ms. What that
  switches off is the work whose cost is the whole document, and all three
  paid it per pause in typing. The number is measured and argued for at the
  constant. The mode is decided when the buffer takes its content and shown in
  the status bar, because a missing squiggle has to read as a decision rather
...
| Nothing can tell whether a command *should* declare a capability | Fixed 2026-08-31 for thirteen commands, and **the class fixed 2026-09-03**: an undeclared command is now refused to a non-user principal, so a missing declaration fails closed instead of opening a door (A7-001, `PermissionService.refuseUndeclared`). The thirteen are still declared, because the rule and the declarations answer different questions: the rule decides what an agent may reach, the declaration decides what the user is asked. The six the review named were joined by seven more of the same kind, found only by reading all 171 by hand: `terminal.focus` opens the panel before it focuses it and so starts a shell, `notes.delete` and `notes.newFromSelection` write, `prefs.reset` rewrites `settings.json`, `search.undoReplace` writes across the project, and `agents.undoLastSession` reverts buffers *and* revokes grants. `view.reloadWindow` declares `permissions.revoke`, which policy denies, because its effect is to erase the in-memory decision log and every grant. What is still not fixed is the sentence this row is named for: there is no way to ask a `run` function whether it reaches the OS, so a command that *should* declare `fs.write` and does not is still wrong for the user, who is exempt from the check. `tests/command-capabilities.test.ts` pins the set of commands declaring nothing, grouped by why, which is what catches that at review rather than at audit. |
...
| A fourth panel in the editor slot costs a line in each of the other three | `showAgents`, `showDiff` and `showWelcome` each clear the other two by name, so the slot is N-by-N. Tasks was the fifth until 2026-09-05, when it moved to the bottom panel (`ui/BottomPanel.svelte`), which took the slot back to four; the shape is unchanged and this row stays. `dismissTop` and `hasDismissible` have the same shape. One `editorLayer: Signal<'review' \| 'agents' \| 'diff' \| 'tasks' \| null>` would collapse all of it, and it is a refactor of four shipped panels rather than something to do while adding the fifth. `tests/overlay-routing.test.ts` covers the overlays and nothing covers this slot, which is the part that would want writing first. |
| ~~Tasks would rather be a bottom panel than an editor one~~ | Fixed 2026-09-05: `ui/BottomPanel.svelte` is the container this row said did not exist, with a tab strip for Terminal and Tasks, one view at a time, below the editor rather than instead of it. `terminal.height` keeps its key and now sizes the panel. What is still true from the original row: Problems and References stay in the sidebar, and the panel has no drag handle yet, so its height is a setting. See `docs/superpowers/specs/2026-09-05-bottom-panel-design.md`. |
...
| Large-file mode is a cliff, and the backup is a compromise inside it | Above `LARGE_FILE_BYTES` (§6) a buffer loses its language server and its gutter diff outright rather than getting a viewport-bounded version of either, which is the same trade as the word fallback below and the same honest fix: bound the work, not the file. The gutter is the nearer of the two, since `diffText` already has a base and a text and could be asked for the visible range. The language server is not, because LSP has no way to describe a document you are only showing part of. The backup is the one that could not be a cliff at all: dropping it would mean the larger the file the less Nox protects unsaved work, so it is written on a 2 s debounce instead, which is a real if bounded widening of the crash window from 400 ms. The mode is also decided when the buffer takes its content, so a file that grows past the line by typing keeps the mode it opened in until it is reloaded or reopened. |
...
| The three process writes still run on the main thread | A plain `#[tauri::command]` body runs inline on the thread that handles the IPC message and draws the window. Until 2026-09-02 that was 48 of 49 commands, and the ones that hurt were the writes: `nox_git_commit` waits for the repository's hooks, `nox_git_switch` checks out a tree, every save calls `sync_all`, and on Linux `nox_watch` walks the whole tree registering inotify watches. All of those, every other `fs.rs` and `git.rs` command, and `nox_read_config`/`nox_write_config` are `#[tauri::command(async)]` now, bodies unchanged. Three stay plain on purpose: `nox_pty_write`, `nox_lsp_send` and `nox_agent_send`. The renderer fires them without awaiting (`services/terminal.ts`, `services/lsp/documents.ts`), and a sync body runs to completion before the next IPC message is read, which is what keeps keystrokes and `didChange` versions in order; under `(async)` each would be a pool task racing for the registry lock. `nox_unwatch` stays sync for the same reason against its own un-awaited disposer. The cost that remains is a full pipe: a multi-megabyte `didChange` into a server that has stopped reading holds the window until the pipe drains. Since 2026-09-03 that message is bounded: `LARGE_FILE_BYTES` (§6) means no document over 5 MB is ever sent to a server at all, so the worst case is 5 MB rather than the 64 MB the open limit allows. Bounded is not fixed. The fix is a writer thread per process fed by a channel, so the command returns after a `send` and order is the channel's. |
...
| Indentation is detected once, at open, and the answer is not persisted | `detectIndentation` (`core/indentation.ts`) runs in `WorkspaceService.open` and nowhere else, so a file that is reloaded from disk, reverted, or rewritten by a formatter keeps the indentation it was read with, and an override the user set by hand is gone at the next launch. Both are deliberate: re-detecting on every external reload would change what a keypress inserts while the user is typing, and persisting the override means a per-path record in `session.json` that has to be invalidated when the file changes underneath it. `.editorconfig` is the other thing not here, and it is not an oversight: the decision taken for A1-004 was per-file detection only. It would arrive through the same seam, as a second source for the `indent` that `StateFactoryArgs` already carries. |
...
| Grouped undo is bounded by CodeMirror's history depth | A change set old enough to have fallen out of a buffer's history cannot be undone as a group. The check is depth plus the document the set left behind, so a trimmed history that brings the depth back around reports "cannot undo as a group" rather than undoing the user's latest edit; the one gap left is a later edit that restores the exact text at the same depth. The project-replace panel's journal covers the fallen-out case for replace. Nothing else needs it yet. |
...
| The agent read boundary does not resolve symlinks | Fixed 2026-09-03 for the case that motivated it: `ContextService.inScope` refuses a non-user principal any buffer whose path falls outside the workspace root, so a `.env` or a `servers.json` open in a tab is no longer listed, quoted in the opening brief, or readable. It asks `containsResolved`, the same helper `PermissionService` asks about `fs.*`, which resolves `..` and not links. A symlink inside the root pointing out of it therefore still reads as inside, and the buffer's path is whatever the file was opened by rather than what it resolves to. Closing that wants a real-path method on `Platform` (`fs.rs` canonicalises only on write and rename, and nothing in the renderer can ask), and it would close the same hole in the permission layer, so it is one change for both rather than a second rule here. Two other limits worth stating plainly: with no folder open every open buffer is in scope, which is what a scratch session needs and what `PermissionService` already does for `fs.*`, and the boundary is on what Nox hands the agent, not on what the agent's own process can reach, which is the row below. |
| Agent processes are not sandboxed | A configured agent runs with Nox's own privileges. The permission model governs what it may ask *Nox* to do, not what its own process can reach. A stdio agent is trusted code you chose to run, like a shell plugin. Its lifetime is tied to Nox's by the OS on Linux and Windows since 2026-09-06 (`src-tauri/src/lifetime.rs`: a death signal, a kill-on-close job object), and by two hooks everywhere: the renderer's `beforeunload` and the host's `RunEvent::Exit` kill every agent, terminal and language server on a clean quit. What remains is macOS after a *crash*: a shell in a pty still dies there because the kernel closes the pty master, but an agent or a language server on pipes does not, and macOS offers no death-signal primitive to close that. |
...
| Ten grammars colour but do not parse | Shell, TOML, Ruby, C#, Kotlin, Swift, Lua, PowerShell, INI and Dockerfile load `@codemirror/legacy-modes` through `StreamLanguage`, which tokenises line by line and builds no tree. They highlight correctly and **Go to Symbol, sticky scroll and syntax folding stay empty in them**, because all three read a parse tree. The palette says so in its own words rather than reporting the file as bare: `hasSymbolStructure` in `editor/languages.ts`, and `symbolListState`'s `no-structure`. No Lezer grammar exists for any of the ten to upgrade to. `LOADERS` is the only place that would change. Makefile has no mode in the package at all, so it still opens as plain text. |
...
| The menu has no Close Window item | `PredefinedMenuItem::close_window` carries ⌘W and Nox binds ⌘W to `file.close`, so both in one menu would be two items claiming one accelerator. `window.close` exists as a command now (with `window.minimize` and `window.toggleMaximize`, so the title bar's buttons go through the registry and a `keybindings.json` rule can reach them), but it is hidden from the palette and listed in no menu: the Window menu already carries the OS's minimise and maximise items, and a close entry would be the accelerator clash above. The traffic light and ⌘Q remain the visible ways out. |
...
| A commit still runs the repository's own hooks, `gpg.program` and `core.fsmonitor` | Every git *read* Nox issues passes `-c core.fsmonitor=false`, so opening or browsing a folder that arrived with its own `.git/config` (an archive, a shared drive; a clone never carries one) runs nothing that file names. `nox_git_commit` deliberately does not carry the override, because hooks and signing are expected on a commit, so a repo-local `core.hooksPath`, `gpg.program` or `core.fsmonitor` still executes there, on an action the user chose. The same exposure every git GUI has; recorded so nobody reads the read-side override as covering the write. |
