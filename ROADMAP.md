...
| **A permission model** | Commands declare what they need. Programmatic callers are checked against a policy, with per-file grants and a decision log. You are never prompted for your own actions; a program's request can be put to you. Nothing uses it yet but its tests, which is why it is right. |
...
| **Nothing in the release notes says "unverified".** | Closing steadily. 2026-08-20 drove the keybinding editor, `inert`, the row-height contract and explorer windowing over 609 nodes in Chromium through the browser target. 2026-08-29 added three walks of the packaged Windows build, including its pixels, plus a browser project that measures real layout. | The part that is genuinely outside the WebView: the menu bar, native dialogs, the terminal, the git panel against a real repo. On a real keyboard, before the 1.0 tag. Not a feature, a ritual. Everything *drawn inside* the WebView can now be checked without the desktop at all, in the `editor` browser project. |
