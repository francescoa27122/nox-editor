...
| **Go to definition** ✅ | `F12`. The same door as hover: `LspService.requestFor` for the question, `workspace.open` and `workspace.setSelection` for the answer, which lands whether or not the pane has swapped to the file yet. Several results take the first and say so; the picker is find references' list, built once for both. |
