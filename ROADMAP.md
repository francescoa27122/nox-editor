...
| **Hover** ✅ | Resting the pointer on a symbol shows its type and documentation, highlighting the span the server names rather than the character under the pointer. The server's markdown is rendered as **text, never HTML** — a language server is a third-party process and its hover strings come from cloned source, so parsing them into a live DOM would buy typography with an injection surface. Inline `**bold**` therefore shows as written. |
